  ...
  return (
    <div className="flex flex-col h-[calc(100vh-140px)] md:h-[calc(100vh-160px)]">
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-2">
        {messages.length === 0 && (
          <p className="text-sm text-c2b-green/40 italic text-center mt-8">
      ...
      <div className="border-t border-c2b-green/10 p-3 flex gap-2 bg-c2b-cream">
        <input
          value={texte}
          onChange={(e) => setTexte(e.target.value)}
          onKeyDown={(e) => e.key === "Enter" && envoyer()}
          placeholder="Votre message..."
          className="flex-1 rounded-full border border-c2b-green/20 px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-c2b-gold"
        />
        <button
          onClick={envoyer}
          className="w-10 h-10 rounded-full bg-c2b-green text-c2b-cream flex items-center justify-center flex-shrink-0"
        >
          <Send size={16} />
        </button>
      </div>
